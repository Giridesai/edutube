...
class YouTubeAPIService {
    ...
    try {
      ...
      return await getOrSetCache(
        ...
        async () => {
          ...
          if (channelIds.length > 0 && this.canMakeRequest(QUOTA_COSTS.channel)) {
            const channelResponse = await this.youtube.channels.list({
              part: ['snippet'],
              id: channelIds,
            });
            this.recordRequest(QUOTA_COSTS.channel);
    ...
    try {
      ...
      return await getOrSetCache(
        ...
        async () => {
          ...
          const channelIds = [...new Set(allVideos.map((item: any) => item.snippet?.channelId).filter(Boolean))]
          ...
          if (uniqueVideoIds.length > 0 && this.canMakeRequest(QUOTA_COSTS.video)) {
            ...
            // Format videos for frontend consumption
            trendingVideos = combinedVideos.map(video => this.formatVideoFromAPI(video, channelThumbnails.get(video.snippet?.channelId)))
  ...
  getQuotaInfo(): { used: number; limit: number; remaining: number; resetTime: number } {
    return {
      ...
      limit: 10000,
      remaining: 10000 - this.quotaUsage.used,
  ...
  async getChannelInfo(channelId: string): Promise<any> {
    try {
      ...
      return await getOrSetCache(
        ...
        async () => {
          // Check API limits
          if (!this.canMakeRequest(QUOTA_COSTS.channel)) {
            throw new Error('YouTube API quota or rate limit exceeded')
          }
          ...
          return {
            ...
            subscribers: this.formatSubscriberCount(statistics?.subscriberCount || '0'),
    ...
    try {
      ...
      return await getOrSetCache(
        ...
        async () => {
          ...
          console.log(`Fetching videos for channel ${channelId} from YouTube API`)
          const searchResponse = await this.youtube.search.list({
            part: ['snippet'],
            channelId: channelId,
            type: ['video'],
          ...
          if (videoIds.length > 0 && this.canMakeRequest(QUOTA_COSTS.video)) {
            ...
            videos = combinedVideos.map(video => ({
              ...
              duration: this.formatDuration(this.parseISO8601Duration(video.contentDetails?.duration || 'PT0S')),
              views: this.formatViewCount(video.statistics?.viewCount || '0'),
              viewCount: parseInt(video.statistics?.viewCount || '0'),
              published: this.formatPublishedDate(video.snippet?.publishedAt || ''),
  ...
  async getChannelPlaylists(channelId: string, maxResults: number = 20): Promise<any[]> {
    try {
      const cacheKey = `channel-playlists:${channelId}:${maxResults}`

      return await getOrSetCache(
        cacheKey,
        async () => {
          if (!this.canMakeRequest(QUOTA_COSTS.playlist)) {
            throw new Error('YouTube API quota or rate limit exceeded')
          }

          console.log(`Fetching playlists for channel ${channelId} from YouTube API`)
          ...
          this.recordRequest(QUOTA_COSTS.playlist)

          const playlists = (response.data.items || []).map((pl: any) => ({
            id: pl.id,
            title: pl.snippet?.title || 'Untitled',
            description: pl.snippet?.description || '',
            thumbnailUrl: pl.snippet?.thumbnails?.medium?.url || pl.snippet?.thumbnails?.default?.url,
